...
class Isoparametrico_Cuad:
    ...
    def __init__(self, n, i, j, k, l, i_node, j_node, k_node, l_node,
        ...
        for i in range(4):
            ...
            t = t_list[i]
            W = W_list[i]
            jacobiano = self.jacobiano(s, t)
            B = self.B(s, t)
            D = self.D
            K_i = np.dot(np.dot(B.transpose(),D),B) * self.t * jacobiano * W * W
            K += K_i
